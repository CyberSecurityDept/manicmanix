...
class Command:
    ...
    def _setup_logging(self):
        ...
        file_handler = logging.FileHandler(
            os.path.join(self.results_path, "cundamanix.log")
        )
        formatter = logging.Formatter(
            "%(asctime)s - [cundamanix] - " "%(levelname)s - %(message)s"
        )
    ...
    def _show_support_message(self) -> None:
        support_message = "Please seek reputable expert help if you have serious concerns about a possible spyware attack. Such support is available to human rights defenders and civil society through Amnesty International's Security Lab."
        if self.detected_count == 0:
            self.log.info(
                f"[bold]NOTE:[/bold] Using public indicators of compromise (IOCs) [bold]WILL NOT[/bold] automatically detect advanced attacks.\n\n{support_message}",
                extra={"markup": True},
            )
        else:
            self.log.warning(
