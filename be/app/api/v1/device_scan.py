...
async def calculate_risk(serial_number: str):
    try:
        ...
        logger.info(f"Memeriksa direktori: {base_dir}")

        
        if not os.path.exists(base_dir):
            raise FileNotFoundError(f"Direktori {base_dir} tidak ditemukan")
        ...
        for folder_name in os.listdir(base_dir):
            ...
            if os.path.isdir(folder_path):  
                possible_task_result_dir = os.path.join(folder_path, "task_result")
                if os.path.exists(possible_task_result_dir):
                    task_result_dir = possible_task_result_dir
                    break
